...
const String SpatGrisAudioProcessor::getParameterName (int index) {
    ...
    if (index < kMaxInputs * kParamsPerSource) {
		String s("Source ");
	...
	index -= kMaxInputs * kParamsPerSource;
    if (index < kMaxChannels * kParamsPerSpeakers) {
		String s("Speaker ");
    ...
    for (int iCurSource = 0; iCurSource < mNumberOfSources; iCurSource++) {
        ...
        for (unsigned int iCurSampleId = 0; iCurSampleId < m_iDawBufferSize; ++iCurSampleId) {
            ...
            if (r >= kThetaRampRadius) {
                ...
                if(!mLockedThetas.isEmpty()){
                    lt = mLockedThetas.getUnchecked(iCurSource);
                }
                float dt = lt - it;
                if (dt < 0) dt = -dt;
            ...
            jassert((t >= 0 && t <= kThetaMax) || (angle > 0 && angle <= M_PI));
